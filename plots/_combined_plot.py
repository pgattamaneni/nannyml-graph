...
def _data_prep_combined_plot(
    data: pd.DataFrame,
    metric_column_name: str,
    metric_column_name_realized: str,
    ...
    data['metric_label_estimated'] = data[metric_column_name].apply(lambda x: hover_metric_format.format(x))
    data['metric_label_realized'] = data[metric_column_name_realized].apply(lambda x: hover_metric_format.format(x))
...
def _combined_plot(
    ...
    chunk_legend_labels_realized=None,
    ...
    if chunk_legend_labels_realized is None:
        chunk_legend_labels_realized = ['Reference period', 'Analysis period']
    ...
    if colors is None:
        colors = [Colors.BLUE_SKY_CRAYOLA, Colors.INDIGO_PERSIAN, Colors.GRAY_DARK, Colors.RED_IMPERIAL, Colors.STRONG_MAGENTA]

    data = _data_prep_combined_plot(
        table,
        metric_column_name,
        metric_column_name_realized,
    ...
    # Plot metric for reference and analysis period
    _plot_metric_realized(
        fig,
        data,
        colors,
        metric_column_name_realized,
        estimated_column_name,
        chunk_legend_labels_realized,
        chunk_type_column_name,
        chunk_types,
        start_date_column_name,
        end_date_column_name,
    ...
    # Plot reference and analysis markers that did not drift
    _plot_non_drifted_markers_realized(
        fig,
        data,
        colors,
        metric_column_name_realized,
    ...
    _plot_drifted_markers_and_areas_realized(
        fig,
        data,
        colors,
        alpha,
    ...
    for i, chunk_type in enumerate(chunk_types):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode='lines',
                x=data_subset[start_date_column_name],
                y=data_subset[metric_column_name_realized],
                line=dict(shape='hv', color=colors[i], width=2, dash=dash),
                hoverinfo='skip',
                legendgroup=1,
            )
...
def _plot_non_drifted_markers_realized(
    fig,
    data,
    colors,
    metric_column_name,
...
def _plot_drifted_markers_and_areas_realized(
    fig,
    data,
    colors,
    alpha,
